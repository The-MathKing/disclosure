...
\begin{itemize}
    \item \textbf{H1:} Higher levels of AI involvement disclosure (None $>$ AI-assisted $>$ AI-generated) will lead to lower perceived source authenticity.
\end{itemize}
...
\subsection{Measures}
Source authenticity was measured using an adapted scale from \citet{morhart2015} ($\alpha = [TBD]$). Consumer trust was measured via \citet{mcknight2002} ($\alpha = [TBD]$), and purchase intention using semantic differentials from \citet{putrevu1994} ($\alpha = [TBD]$). Manipulation checks for product typology adapted \citet{nelson1970}.
...
Furthermore, a manipulation check will evaluate the product typology paradigm. Pending real data collection, we expect participants to rate the facial serum as significantly higher on experiential dimensions ($M = [TBD]$, $SD = [TBD]$) than the USB flash drive ($M = [TBD]$, $SD = [TBD]$), $t([TBD]) = [TBD]$, $p < .001$.
